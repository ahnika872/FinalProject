...
TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3VzZXItcmlnaHQiLCJzdWIiOjk2MzM1NiwiaWF0IjoxNzMxMzYyNzM1LCJleHAiOjE3MzEzNjYzMzUsInR5cGUiOjIwfQ.VMzd66OQNc8XSyA_2hMk_Px3aWHf6Gyq6zwjXvcO9xs"
...
def test_update_product_quantity():
    with allure.step("Изменение количества товара в корзине"):
        ...
        response = requests.post(f"{BASE_API_URL}/product", json={"id": 2651550}, headers=HEADERS)
...
def test_get_cart_total():
    with allure.step("Получение общей стоимости товаров в корзине"):
        response = requests.post(f"{BASE_API_URL}/product", json={"id": 2651550}, headers=HEADERS)
        response = requests.put(f"{BASE_API_URL}", json=[{"id":153459253, "quantity": 2}], headers=HEADERS)
        ...
        total_quantity = sum(item["quantity"] for item in cart_items)  # Подсчет общего количества товаров
